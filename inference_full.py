...
def generate_molecules(df, pipeline, prompt_template, task_id, prop_ranges, prop_trends):
        ...
        for i in range(num_prompts):
            ...
            for j, prop in enumerate(task_props):
                ...
                if prop_trends[task_id][j] == "1":
                    ...
                    if prop == 'qed' and desired_value > 1: desired_value = random.uniform(0.97, 0.99)
                    temp.append(f'|{prop}|{round(desired_value,4)}')
                else:
                    desired_value = prop_values[prop] - prop_ranges_value - random.uniform(0,0.1)
                    if desired_value <= 0:
                        if prop in ['NumHAcceptors', 'NumHDonors']: desired_value = 1.0
                        elif prop in ['qed']: desired_value = 0.1
                        elif prop in ['TPSA']: desired_value = 10.0
                    temp.append(f'|{prop}|{round(desired_value,4)}')
            prompt = ', '.join(temp)
            prompt = prompt
    ...
    def extract_smiles(text):
        if args.tune_model.count('301') != 0:
            res = text.split('[START_I_SMILES]')[-1].split(',')[0].split(' ')[0].strip(" ,.:\"")
            return res
        if args.tune_model.count('general') != 0:
            res = text.split('The recover molecule is [START_SMILES]')[-1].split(',')[0].strip(" ,.:\"")
        else:
            res = text.split('[START_SMILES]')[-1].split(',')[0].split(' ')[0].strip(" ,.:\"")
        return res
    ...
    for idx in tqdm(range(df.shape[0])):
        prop_values = {prop: df[prop][idx] for prop in ["MolLogP", "qed", "TPSA", "NumHAcceptors", "NumHDonors", "MolWt"]}
        prop_values['DRD2'] = 0.5
        scaffold, mol = df['scaffold'][idx], df['mol'][idx]
        if scaffold in ['c1cscn1','c1ccccc1']:
            scaffold = df['mol'][idx]
        source_mol = df['mol'][idx]
        mol = copy.deepcopy(source_mol)
        prop_prompts = generate_property_prompts(task_id, prop_values, prop_ranges, prop_trends, args.k)

        gene_mol_set = set()
        ans_set = set()
        
        # Randomly select a prompt template for each iteration
        for prop_prompt in prop_prompts:
            ...
            print('gene:', gene_molecule, 'scaffold', scaffold_generate, 'same',same_scaffold(gene_molecule, scaffold_source))
            if scaffold_generate != '':
                gene_mol_set.add(gene_molecule)
            ans_set.add(answer)
            scaffold = get_alternative_smiles(df['mol'][idx])
            if len(list(gene_mol_set)) >= 20:
                break
        with open(f'./generated_new/full_{dataset_type}_{task_id}_{args.backbone}_{args.tune_model}.csv','a+')as f:
...
def get_args():
    ...
    parser.add_argument('--k', type=int, default=8, help='Number of generated molecules')
...
if __name__ == "__main__":
    ...
    if args.tune_model == '301':
         prompt_template = config['inference_template_molecule_301']
    elif args.tune_model.count('general') == 0:
        prompt_template = config['inference_template_molecule']
    else:
        prompt_template = config['inference_template_general']
    ...
    df.to_csv(f'./output/{dataset_type}_{args.backbone}_{args.tune_model}_{task_id}_{args.k}.csv', index=False)
    ...
    with open('/inspire/hdd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/yujiajun-240108120114/llm4drug/output/log/base_submit.log', 'a+') as log_file:
        log_file.write(f"{args.test_data},{args.backbone},{args.tune_model},{task_id},{args.k},{loose_hit:.4f},{strict_hit:.4f},{valid_ratio:.4f},{same_scaffold_ratio:.4f},{similarity:.4f}\n")
